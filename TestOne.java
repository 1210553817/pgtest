...
public class TestOne {
	...
	public static void doExport(String arrs, OutputStream os) throws Exception {
		...
		Map<String, CellStyle> styles = styles(wb);
		CellStyle headSty = styles.get("header");
		int m=0;
		for (JsonElement data : dats) {
			...
			int i = 0, j;
			Map<Integer,Integer> spx = new HashMap<Integer, Integer>();
			Map<Integer,Integer> spy = new HashMap<Integer, Integer>();
			for (JsonElement ce : cols) {
				...
				for (JsonElement cm : ces) {
					...
					addr = new CellRangeAddress(i, i+rsp-1, j, j + csp-1);
					s.addMergedRegion(addr);
					c = r.createCell(j);
					c.setCellStyle(headSty);
					s.setColumnWidth(j, 3000);
					c.setCellValue(tit);

					for (int x = 0; x < csp; x++) {
						for (int y = 0; y < rsp; y++) {
							Object xo = spx.get(j+x);
							int xp = xo!=null?spx.get(j+x):1;
							spx.put(j+x, ++xp);
							Object yo = spx.get(i+y);
							int yp = yo!=null?spx.get(i+y):1;
							spy.put(i+y, ++yp);
