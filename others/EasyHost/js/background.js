    ...
    if(socktId==info.socketId){
		if(1==mailStatus){
			...
			if(data.indexOf("235")>-1){
				mailStatus =5;
				chrome.sockets.tcp.send(socktId, str2ab("mail from:<"+mailReq.from+">\r\n"), function(infod){});
			}
		}else if(5==mailStatus){
			if(data.indexOf("250")>-1){
				mailStatus =6;
				chrome.sockets.tcp.send(socktId, str2ab("rcpt to:<"+mailReq.to+">\r\n"), function(infod){});
			}
		}else if(6==mailStatus){
			if(data.indexOf("250")>-1){
				mailStatus =7;
				chrome.sockets.tcp.send(socktId, str2ab("data\r\nsubject:"+mailReq.tit+"\r\n\r\n"+mailReq.txt+"\r\n.\r\n"), function(infod){});
			}
		}else if(7==mailStatus){
			if(data.indexOf("250")>-1){
				mailStatus =8;
...
function closeSockt(sid){
	chrome.socket.tcp.close(sid, function(){});
}
// 创建Base64对象
